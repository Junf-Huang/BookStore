...
public class OneApplication {
	...
	public void setDefault(Book book, String userId, int number, OrderRepository orderRepository){
            Order order = new Order();
	        order.setUserId(userId);
            order.setBookId(book.getId().toString());
            order.setBookName(book.getBookName());
            order.setNumber(number);
            order.setBalance(true);
            order.setPrice(book.getPrice()*number);
            orderRepository.save(order);
    }

    @Bean
    CommandLineRunner init(BookRepository bookRepository, UserRepository userRepository, OrderRepository orderRepository) {
        return (args) -> {
            //init implement
            //UserTestData
            ...
            //repeat too much , please write function
            Book book= new Book("9787020026906", "白鹿原","陈忠实 ","文学", "这是一部渭河平原五十年变迁的雄奇史诗，一轴中国农村班斓多彩、触目惊心的长幅画卷。主人公六娶六丧，神秘的序曲预示着不祥。一个家族两代子孙，为争夺白鹿原的统治代代争斗不已，上演了一幕幕惊心动魄的活剧：巧取风水地，恶施美人计，孝子为匪，亲翁杀媳，兄弟相煎，情人反目……大革命、日寇入侵、三年内战，白鹿原翻云覆雨，王旗变幻，家仇国恨交错缠结，冤冤相报代代不已……古老的土地在新生的阵痛中颤栗。厚重深邃的思想内容，复杂多变的人物性格，跌宕曲折的故事情节，绚丽多彩的风土人情，形成作品鲜明的艺术特色和令人震撼的真实感。");
            book.setBookPhoto(book.getBookISBN()+".jpg");
            book.setPrice(28.00);
            bookRepository.save(book);
            setDefault(book, one.getStuID(),1, orderRepository);

            book= new Book("9787020044528","平凡的世界", "路遥 ","文学", "这是一部全景式地表现中国当代城乡社会生活的长篇小说。全书共三部。作者在近十年间广阔背景上，通过复杂的矛盾纠葛，刻划了社会各阶层众多普通人的形象。劳动与爱情，挫折与追求，痛苦与欢乐，日常生活与巨大社会冲突，纷繁地交织在一起，深刻地展示了普通人在大时代历史进程中所走过的艰难曲折的道路。");
            book.setBookPhoto(book.getBookISBN()+".jpg");
            book.setPrice(60.00);
            bookRepository.save(book);
            setDefault(book, one.getStuID(),2, orderRepository);


            book= new Book("9787020056309","东方快车谋杀案", "阿加莎·克里斯蒂","悬疑", "午夜过后，一场大雪迫使东方快车停了下来。这辆豪华列车整年都处于满员状态。但那天早上却发现少了一名乘客。一个美国人死在了他的包厢里，他被刺了十二刀，可他包厢的门却是反锁着的。");
            book.setBookPhoto(book.getBookISBN()+".jpg");
            book.setPrice(18.00);
            bookRepository.save(book);
            setDefault(book, one.getStuID(),3, orderRepository);

            book= new Book("9787020065394","无人生还", " 阿加莎·克里斯蒂 ","悬疑", "十个互不相识的人，被富有的欧文先生邀请到了印地安岛上的私人别墅里。晚餐后，一个神秘的声音揭开了人们心中所各自隐藏着的可怕秘密。当天晚上，年轻的马斯顿先生离奇死去，古老的童谣就像诅咒一样笼罩着所有人，似乎有一双神秘的眼镜在时刻窥视着这场死亡游戏，到访者就像消失的印地安小瓷人一样一个又一个的走向死神。");
            book.setBookPhoto(book.getBookISBN()+".jpg");
            book.setPrice(19.00);
            bookRepository.save(book);
            setDefault(book, one.getStuID(),1, orderRepository);
            ...
            //two buy
            setDefault(book, two.getStuID(),1, orderRepository);

            List<User> users = new ArrayList<User>();
            users.add(one);

            //这里设置了角色,ManyToMany没用到
            one.setRoles(roles1);
            //User开启了级联，当保存用户的同时也保存了书本信息
            ...
            //BookTestData
            book = new Book("9787562017677","法理学 : 法律哲学与法律方法"," E.博登海默","本书把散见于1940年《法理学》一书中的有关法理学思想发展的历史资料集中在第一部分。本书第二部发和第三部分中对一般法律理论的实质性问题所作的论述，乃是以某些蕴含在我研究法理学问题的进路中的哲学假设和方法论假设为基础的。");
